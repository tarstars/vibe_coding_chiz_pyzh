...
4. Во время проигрывания подсвечивает текущую ноту в круге и рисует внешний сектор толщиной, пропорциональной длительности; при повторе той же ноты сектор накапливается, а новый слой добавляется с внутренней стороны (к центру).
...
5. Мелодия проигрывается однократно (без повтора фразы).
6. Накопление внешних секторов ведется в рамках одного запуска `Play`; при новом запуске визуальный стек секторов очищается.
...
5. Добавить состояния:
   ...
   - накопительный стек слоев для каждого pitch class: при повторном попадании в тот же сектор добавляется новый слой с внутренней стороны уже нарисованных.
6. Зафиксировать правило геометрии накопления:
   - первый слой ноты рисуется у внешней границы кольца,
   - каждый следующий слой той же ноты рисуется ближе к центру (inward growth), не перезаписывая предыдущие.
...
5. После завершения каждой ноты обновлять стек ее сектора: фиксировать добавленный слой и сохранять ранее нарисованные слои.
...
1. Ручные сценарии:
   ...
   - Повторяющиеся ноты (например, `G G G`) дают 3 накопленных слоя в одном секторе, и каждый следующий слой расположен внутри предыдущего.
...
4. Во время звучания ноты подсвечивается нужный сектор и отображается внешний сектор с толщиной по длительности; повторные ноты накапливают слои в том же секторе с ростом внутрь.
